...
public class ElasticConnection {
    ...
    public static ElasticConnection getSmsConnection(EsProperties esProperties) {
        if (smsElasticConnection == null) {
            synchronized (ElasticConnection.class) {
                if (smsElasticConnection == null) {
                    try {
                        String servers = esProperties != null ? esProperties.getServers() : ES_SERVER;
                        ...
                        if (Strings.isNullOrEmpty(servers)) {
                            ...
                            RestHighLevelClient highLevelClient = new RestHighLevelClient(builder);
                            smsElasticConnection =  new ElasticConnection(highLevelClient,client);
                            Runtime.getRuntime().addShutdownHook(new Thread(() -> smsElasticConnection.close()));
                        }
                    } catch (Exception ex) {
                        ex.printStackTrace();
                    }
